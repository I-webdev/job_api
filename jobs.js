...
app.post("/", (req, res) => {
  ...
  jobs.push(jobToBeAdded);
  
});
app.patch("/:id", (req, res) => {
  const _id = req.params.id;
  const existingJob = jobs.find(job => job.id === _id);
  const jobToBeReplaced = {
    id: _id,
    title: req.body.title || existingJob.title,

    type: req.body.type || existingJob.type,
    ...
    company: {
      name: req.body.company.name || existingJob.company.name,
      description:
        req.body.company.description || existingJob.company.description,
      contactEmail:
        req.body.company.contactEmail || existingJob.company.contactEmail,
      contactPhone:
        req.body.company.contactPhone || existingJob.company.contactEmail,
    },
  };
  const searchIndex = jobs.findIndex(job => job.id === _id);
  console.log(jobToBeReplaced);
  
  
  searchIndex > -1 && jobs.splice(searchIndex, 1, jobToBeReplaced);
  // jobs[searchIndex] = jobToBeReplaced;
...
app.delete("/:id", (req, res) => {
  const _id = req.params.id;
  const searchIndex = jobs.findIndex((job)=>job.id === _id);
  searchIndex > -1 && jobs.splice(searchIndex, 1);
});
